...
# All rights reserved.
#
# The classes below are not "complete" - they cannot be instantiated
# directly and need ot be combined with a unittese TestClass class until they
# can be used. Thus, they appear on the surface to access properties that
# do not exist. We silence those warnings because the tests all access
# properties that will exist, but by a user outside this module.
#
...
class CompanionAppServiceRoutesV1(object):
    ...
    def test_get_content_data_video_app_cancel(self, quit_cb):
        '''/v1/content_data when cancelled returns error.'''
        ...
        self.service = CompanionAppService(Holdable(),
                                           ...
                                           FakeContentDbConnection(FAKE_SHARD_CONTENT),
                                           middlewares=[
                                               cancel_route_middleware('/v1/content_data')
                                           ])
        fetch_first_content_id('org.test.ContentApp',
                               ['First Tag'],
                               self.port,
